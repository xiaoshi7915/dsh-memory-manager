...
check('schemas 覆盖 7 工具', TOOL_NAMES.every((n) => schemaNames.includes(n)), `schemas=${JSON.stringify(schemaNames)}`)

// ---------- B) 全链路执行 ----------
const addRes = await ctx.tools.execute({
  callId: 'v-add-1', name: 'memory_add', signal: sig(10000),
  arguments: { content: '我喜欢用 Python 做数据分析，常用 pandas 和 matplotlib。', tags: ['偏好', '技术'], importance: 7, session_id: 'sess-A' },
})
check('execute memory_add 成功', addRes.isError === false, JSON.stringify(addRes).slice(0, 200))
check('memory_add value 有 memory_id', !addRes.isError && !!addRes.value?.memory_id)
check('memory_add 渲染含中文', !addRes.isError && addRes.content.some((b) => b.type === 'text' && b.text.includes('已保存记忆')))
const addVal = addRes.value

await new Promise((r) => setTimeout(r, 30))
const searchRes = await ctx.tools.execute({
  callId: 'v-search-1', name: 'memory_search', signal: sig(10000),
  arguments: { query: '我平时用什么语言做数据分析', session_id: 'sess-A' },
})
check('execute memory_search 成功', searchRes.isError === false, JSON.stringify(searchRes).slice(0, 200))
const hit = searchRes.isError ? null : searchRes.value?.results?.find((r) => r.id === addVal.memory_id)
check('memory_search 召回新增记忆', !!hit && hit.score > 0.4, `hit=${JSON.stringify(hit)}`)
...
const recentRes = await ctx.tools.execute({
  callId: 'v-recent-1', name: 'memory_get_recent', signal: sig(10000),
  arguments: { session_id: 'sess-A' },
})
check('execute memory_get_recent 成功', recentRes.isError === false, JSON.stringify(recentRes).slice(0, 200))
...
const sumRes = await ctx.tools.execute({
  callId: 'v-sum-1', name: 'memory_summarize', signal: sig(10000),
  arguments: { session_id: 'sess-A' },
})
check('execute memory_summarize 成功', sumRes.isError === false, JSON.stringify(sumRes).slice(0, 200))
check('memory_summarize 经 LLM 生成', !sumRes.isError && !!sumRes.value?.memory_id && sumRes.value.summary.includes('Python'),
...
const delEmpty = await ctx.tools.execute({
  callId: 'v-del-0', name: 'memory_delete', signal: sig(10000),
  arguments: { session_id: 'sess-A' },
})
check('memory_delete 空目标被守卫拦截', delEmpty.isError === true, JSON.stringify(delEmpty).slice(0, 200))
...
const delRes = await ctx.tools.execute({
  callId: 'v-del-1', name: 'memory_delete', signal: sig(10000),
  arguments: { ids: delIds, session_id: 'sess-A' },
})
check('execute memory_delete 成功', delRes.isError === false, JSON.stringify(delRes).slice(0, 200))
check('memory_delete 清理指定记忆', !delRes.isError && delRes.value?.deleted_count >= 2, `deleted=${JSON.stringify(delRes.value)}`)

const statsRes = await ctx.tools.execute({
  callId: 'v-stats-1', name: 'memory_stats', signal: sig(10000),
  arguments: {},
})
check('execute memory_stats 成功', statsRes.isError === false, JSON.stringify(statsRes).slice(0, 200))
check('memory_stats 字段完整', !statsRes.isError && 'embedding_status' in statsRes.value && 'storage_size_mb' in statsRes.value)

const add2 = await ctx.tools.execute({
  callId: 'v-add-2', name: 'memory_add', signal: sig(10000),
  arguments: { content: '季度目标：Q3 完成模型性能调优', importance: 5, session_id: 'sess-A' },
})
const impRes = await ctx.tools.execute({
  callId: 'v-imp-1', name: 'memory_update_importance', signal: sig(10000),
  arguments: { memory_id: add2.value.memory_id, score: 9 },
})
check('execute memory_update_importance 成功', impRes.isError === false, JSON.stringify(impRes).slice(0, 200))
check('memory_update_importance 更新生效', !impRes.isError && impRes.value?.new_score === 9 && impRes.value?.old_score === 5)
