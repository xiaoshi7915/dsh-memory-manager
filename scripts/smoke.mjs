...
async function main() {
  ...
  // ---- 7 个工具 handler（P1：mm_* 前缀名，HANDLERS 双键指向同一实现） ----
  const add = await HANDLERS.mm_add({ content: '我的宠物猫叫咪咪', tags: ['生活'], importance: 8 }, ctx)
  r(check('mm_add', add.success && add.memory_id && typeof add.embedding_status === 'string' && add.token_cost >= 0, JSON.stringify(add)))

  const addGlobal = await HANDLERS.mm_add({ content: '我喜欢用 Python 做数据分析', tags: ['偏好'], importance: 9, is_global: true }, ctx)
  r(check('mm_add(global)', addGlobal.success === true, JSON.stringify(addGlobal)))

  const search = await HANDLERS.mm_search({ query: '我的猫叫什么名字', top_k: 3 }, ctx)
  r(check('mm_search', Array.isArray(search.results) && search.total >= 1 && typeof search.latency_ms === 'number',
  ...
  const recent = await HANDLERS.mm_get_recent({ n: 5 }, ctx)
  r(check('mm_get_recent', recent.messages.length >= 1 && typeof recent.token_count === 'number' && recent.window_size === recent.messages.length, JSON.stringify(recent)))

  const sum = await HANDLERS.mm_summarize({}, ctx)
  r(check('mm_summarize', sum.summary && sum.memory_id && typeof sum.saved_tokens === 'number', JSON.stringify(sum)))

  const imp = await HANDLERS.mm_update_importance({ memory_id: add.memory_id, score: 10 }, ctx)
  r(check('mm_update_importance', imp.old_score === 8 && imp.new_score === 10, JSON.stringify(imp)))

  const del = await HANDLERS.mm_delete({ conditions: { tag: '生活' } }, ctx)
  r(check('mm_delete', del.deleted_count >= 1 && Array.isArray(del.affected_sessions) && del.freed_tokens >= 0, JSON.stringify(del)))
  ...
  const stats = await HANDLERS.mm_stats({}, ctx)
  r(check('mm_stats', typeof stats.total_memories === 'number' && typeof stats.storage_size_mb === 'number', JSON.stringify(stats)))
  ...
  r(check('触发词-记住', t1?.tool === 'mm_add' && t1.args.content === '我的生日是 5 月 20 号', JSON.stringify(t1)))
  r(check('触发词-检索', t2?.tool === 'mm_search' && t2.args.query === '我喜欢喝茶', JSON.stringify(t2)))
  r(check('触发词-总结', t3?.tool === 'mm_summarize', JSON.stringify(t3)))
  r(check('触发词-打开面板', isOpenPanel('查看我的记忆') === true, JSON.stringify(t4)))
  r(check('触发词-删除', t5?.tool === 'mm_delete' && t5.args.conditions?.tag === '工作', JSON.stringify(t5)))
  r(check('触发词-上下文', t6?.tool === 'mm_get_recent', JSON.stringify(t6)))
