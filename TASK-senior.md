...
## 必须实现
1. **7 个工具注册**（P1 对外名 `mm_*`）：mm_add / mm_search / mm_get_recent / mm_summarize / mm_delete / mm_update_importance / mm_stats。注册方式与参数 schema 严格按 DSH 约定（dsh-research.md 的证据）与 contracts/tools.md。handler 为薄层：入参校验 → 调 core → 映射为契约规定返回结构；错误统一映射为错误码（EMBEDDING_UNAVAILABLE/NOT_FOUND/VALIDATION_ERROR/STORAGE_LIMIT 等），错误信息中文。旧名 `memory_*` 仅在 `compat.bareSearch=true` 时 best-effort 注册为别名。
