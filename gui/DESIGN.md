...
- **统一入口**：在 DSH 设置侧边栏提供「记忆管理」入口（**已实现**：客户端 half `src/client/bundle.js` 注入 `settings.section` 槽位，iframe 内嵌本 GUI，同源 `/api/memory/*` 直连；设计初稿曾写 `sidebar.settings`，实际落地方案为 `settings.section`——即设置页左侧导航），与 Agent 的工具调用（`mm_add` / `mm_search` 等）共用同一记忆库。
